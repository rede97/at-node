/*
  ESP32-C3 BLE HID host probe — reference host for AT-Node testing.

  Purpose:
    Acts as a known-good BLE central to validate AT-Node's BLE keyboard
    (Peripheral) mode, and as the reference host flow for the dongle
    (BLE HID receiver) implementation on branch dongle-wip.

  Behavior:
    1. Scans for "AT-Node" advertising the HID service (0x1812).
    2. Connects with security (Bluedroid initiates on-connect; static
       passkey 123456, MITM on — AT-Node accepts Just Works).
    3. Dumps all GATT services/characteristics to serial.
    4. Subscribes the first HID Report characteristic (0x2A4D) and the
       Battery Level characteristic (0x2A19).
    5. Prints every notification as hex. Expected keyboard reports are
       the standard 8-byte boot layout (mods, reserved, 6 keycodes) —
       drive them from the AT-Node side with AT+KEY over USB CDC, e.g.
       AT+KEY=0,4  (key 'a' down) / AT+KEY=0  (release).

  Build (arduino-cli, needs USB CDC for serial output!):
    arduino-cli compile --fqbn esp32:esp32:esp32c3:CDCOnBoot=cdc esp32c3_probe
    arduino-cli upload -p COMx --fqbn esp32:esp32:esp32c3:CDCOnBoot=cdc esp32c3_probe
    Then open serial monitor at 115200.

  Verified 2026-07: connects to AT-Node keyboard firmware, receives
  8-byte reports on 0x2A4D for each AT+KEY press/release.
*/
...
static const String DeviceName = "AT-Node";
...
bool connectToServer() {
...

  {
    ...
    if (pRemoteKeyboardCharacteristic == nullptr) {
      Serial.print("Failed to find secure characteristic UUID: ");
      Serial.println(KEYBOARD_secureCharUUID.toString().c_str());
      pClient->disconnect();
      return false;
    }
    Serial.println(" - Found secure characteristic");

    if (pRemoteKeyboardCharacteristic->canNotify()) {
