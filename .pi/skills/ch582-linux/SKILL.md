...
关键点:`wchble/mr2/obj/` 的 makefile 硬编码调用 `riscv-none-embed-gcc`,
...
```bash
source env.sh
cd wchble/mr2/obj && make --no-print-directory main-build
...
已批量改为相对路径(make 始终在 `wchble/mr2/obj/` 下执行,`..` = `wchble/mr2/`,
Windows/Linux 通用):
...
```bash
cd wchble/mr2/obj
sed -i 's|e:/Projects/at_node/wchble/mr2|..|g' */subdir.mk APP/*/subdir.mk makefile
```
...
wlink flash wchble/mr2/obj/at-node.hex    # 烧录 + 自动复位,162 KB 约 18 秒
