...
6. **REFACTOR_PLAN.md 进度追踪。** 重构中的项目，AI 读一眼就知道哪些符号已迁移、哪些还在过渡期，避免在旧代码上浪费 token。
...
│   │   ├── hidkbd.h               # BLE 键盘事件定义（过渡，将合并到 BLE）
│   │   └── usb_dev.h              # USB 设备接口
│   ├── HWS/                       # 硬件服务层
│   │   ├── hws.h                  # 伞形头文件
...
```c
int main(void) {
    SetSysClock(CLK_SOURCE_PLL_60MHz);     // 1. 时钟 60MHz
    UART1_DefInit();                        // 2. 调试串口
    ble_stack_init();                       // 3. BLE 协议栈配置
    hws_init();                             // 4. 硬件服务（KEY/LED/RTC/SLEEP）
    at_init(cmd_table, cmd_table_count);    // 5. AT 命令解析器
    hws_key_config(key_press);              // 6. 按键回调注册
    GAPRole_PeripheralInit();               // 7. BLE GAP 角色
    ble_hid_dev_init();                     // 8. HID Device Service
    ble_hid_emu_init();                     // 9. HID 键盘模拟 + 广播
    #if HWS_SLEEP == FALSE
        USB_Device_Setup();                 // 10. USB 复合设备
    #endif
    while(1) TMOS_SystemProcess();          // 11. 进入 TMOS 主循环
}
```

**初始化顺序是精心设计的：** BLE 协议栈必须在 HWS 之前初始化（HWS 的校准任务依赖 BLE 库函数）。HWS 必须在按键配置之前初始化。AT 解析器独立于上述两者，可以在任意位置。
