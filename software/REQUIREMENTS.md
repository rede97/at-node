...
> **实现现状快照（2026-07-21)**:kbd 全链路生产态；**dongle 接收器 boot 链路已双板验证**（扫描→配对→GATT 发现→订阅→USB 转发，loop test 3 连过）;dongle-wip 已合并 main,`BLE_DONGLE` 编译期切换。
> **阶段二硬化双板验证通过（2026-07-21)**：断链状态全复位、`AT+BT_LIST`、`BLE_DONGLE_DEBUG` 诊断门控、自动回连（`AT+BT_AUTO`，实测改为直连绑定地址；白名单路径在 WCH 库上不可靠已放弃）。测试固化：`tools/test_dongle_hardening.py`。
...
| 编号 | 需求 | 优先级 | 状态 |
|------|------|--------|------|
| F1.16 | `BLE_MODE` 三态宏接入 config.h（替代现有 `BLE_DONGLE` 布尔宏）| P3 | 🚧 已实现待实测（KBD/DONGLE/DUAL，派生 `BLE_HAS_KBD/DONGLE` 门控）|
| F1.17 | DONGLE 构建：Central 角色扫描/连接/绑定 BLE 键盘 | P3 | ✅ 双板验证（含 Just Works/自动配对码） |
| F1.18 | GATT client 解析 HID over GATT 报告（Report Map 解析或 Boot 模式）| P3 | ✅ Boot 模式已验证；Report fallback 已编码，待 RK 真机 |
...
| F1.21 | DUAL 构建：`AT+ROLE` 运行期切换（DataFlash 标志 + 软复位）| P3 | 🚧 已实现待实测（`make MODE=DUAL`，标志存 0x7C00，BT 命令按运行期角色分发）|

> **范围声明（2026-07-22）**：当前阶段接收器**仅要求 Just Works 配对**
> （NoInputNoOutput，静默加密）。完整标准键盘支持（Report Map 解析、
> NKRO 位图转换、Report ID 分派、MITM 密码输入路径）统一降为 **P4
> 最低优先级**，见 F1.22。RK 复盘见 PLAN.md §3.0 —— 复杂多功能键盘
> （多 Report ID/NKRO）在 F1.22 完成前不保证可用。

| 编号 | 需求 | 优先级 | 状态 |
|------|------|--------|------|
| F1.22 | 全量 HOGP 主机：读 Report Map(0x2A4B)+ Report Reference(0x2908)，按 Report ID 分派（boot8 直转 / NKRO 位图→6 键 / 消费控制），MITM 密码配对路径 | **P4（最低）** | 📋 方案已记录（PLAN.md §3.0/§3.5），暂缓实施 |

**AT 命令组（DONGLE 构建，✅=已实现）：**

```
AT+BT_SCAN[=<秒>]     ✅ 扫描 BLE 设备,返回 idx/地址/RSSI/名称
AT+BT_CONN=<idx>      ✅ 连接扫描结果(自动配对/绑定/GATT 发现/订阅)
...
| `AT+ISP` | 进入 ROM ISP bootloader（擦 page0 软复位，wchisp 烧录） | 无 | P2 | 🚧 已实现未实测 |
| `AT+ROLE[=KBD\|DONGLE]` | 查询/切换 BLE 角色（DUAL 构建：写标志 + 软复位） | 无 | P3 | 🚧 已实现待实测 |
...
| `AT+BT_CONN=<idx>` | 连接扫描结果（dongle） | idx: 扫描列表索引 | P3 | ✅ |
| `AT+BT_DISC` | 断开 BLE 链路（kbd=断主机；dongle=断键盘） | 无 | P1 | ✅ |
| `AT+BT_PAIR` | 断开并清除全部绑定（kbd） | 无 | P1 | ✅ |
...
| `AT+IR=<sub>,<args>` | 红外发射（sub=NEC\|SIRC\|RAW） | 见 §3.11 | P2 | 🚧 stub |
...
| F5.3 | 支持通过虚拟串口进行固件升级（DFU） | P2 | 🚧 `AT+ISP` 已实现未实测（配合 wchisp） |
