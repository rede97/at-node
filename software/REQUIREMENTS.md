...
| 编号 | 需求 | 优先级 |
|------|------|--------|
| F1.16 | `BLE_MODE` 三态宏接入 config.h（替代现有 `BLE_DONGLE` 布尔宏）| P3 |
| F1.17 | DONGLE 构建：Central 角色扫描/连接/绑定 BLE 键盘 | P3 |
| F1.18 | GATT client 解析 HID over GATT 报告（Report Map 解析或 Boot 模式）| P3 |
| F1.19 | 收到的键值经 `kb_usb_send_report()` 转发给 PC | P3 |
| F1.20 | DONGLE 构建裁掉 GATT server 服务与广播代码（`#if` 门控）| P3 |
| F1.21 | DUAL 构建：`AT+ROLE` 运行期切换（DataFlash 标志 + 软复位）| P3 |

**AT 命令组（DONGLE 构建）：**

```
AT+BT_SCAN=<秒>      → 扫描周边 BLE 设备，返回 地址/名称/RSSI 列表
AT+BT_CONN=<地址>    → 连接选中的键盘（自动走绑定流程）
