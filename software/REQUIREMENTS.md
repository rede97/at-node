...
| `AT+IR=NEC,<hex>` | 发送 NEC 红外编码 | hex: 32-bit 码值 | P2 |
| `AT+IR=SIRC,<hex>,<bits>` | 发送 Sony SIRC 红外编码 | hex, bits: 12/15/20 | P3 |
| `AT+IR=RAW,<t1>,<t2>,...` | 发送原始红外时序 | ti: mark/space us | P2 |
...
Nordic UART Service (NUS) over BLE — 替代经典蓝牙 SPP。Agent 无线发送 AT 命令。

| 编号 | 需求 | 优先级 |
|------|------|--------|
| F12.1 | 添加 Generic GATT/TX/RX Service (UUID: 6E400001-B5A3-F393-E0A9-E50E24DCCA9E) | P2 |
| F12.2 | RX Characteristic (Write) 接收 Agent 下发的 AT 命令，写入 AT 解析器 | P2 |
| F12.3 | TX Characteristic (Notify) 发送 AT Response 回 Agent | P2 |
| F12.4 | 与 USB CDC 共用 AT 解析器，响应只发来源通道 | P2 |
| F12.5 | App 端可用 nRF UART / Web Bluetooth API 连接 | P3 |

```
Agent (BLE NUS) --Write-> RX Char -> AT parser -> TX Char --Notify-> Agent
Agent (USB CDC) --Write-> USB CDC -> AT parser -> USB CDC --Write-> Agent
                          ↑ 共用同一个 AT 解析器 ↑
```
...
| 编号 | 需求 | 优先级 |
|------|------|--------|
| T6.1 | 当工程宏 `HAL_SLEEP = TRUE` 时，**禁止启用 USB**，仅通过硬件串口（UART）与上位机通信 | P0 |
| T6.2 | 当 `HAL_SLEEP = FALSE` 时，USB 正常可用；若需休眠，由 AT 命令 `AT+SLEEP` 显式触发 | P0 |
| T6.3 | `AT+SLEEP` 命令执行前须先**关闭 USB**（清除 `R8_UDEV_CTRL.port_en`、断开 `DP_PU` 上拉），休眠结束后由上位机重新发起 USB 枚举 | P1 |
| T6.4 | 设备启动时检测 `HAL_SLEEP` 宏：若启用，`USB_Device_Setup()` 直接返回不初始化 | P0 |
| T6.5 | CH592 等后续芯片若支持休眠态保持 USB 连接，可解除此互斥 | P3 |

**典型配置组合**：

| 场景 | HAL_SLEEP | USB | 通信方式 |
|------|-----------|-----|---------|
