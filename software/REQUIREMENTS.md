...
> **阶段二硬化已编码（commit fea1dc0，待双板实测）**：断链状态全复位、`AT+BT_LIST`、`BLE_DONGLE_DEBUG` 诊断门控、白名单自动回连（`AT+BT_AUTO`)。
...
AT+BT_AUTO[=0|1]      🚧 自动回连绑定键盘(白名单+RL,已实现待实测)
AT+BT_LIST            🚧 已绑定设备列表(SNV,已实现待实测)
```
...
| `AT+BT_AUTO[=0\|1]` | 自动回连绑定键盘；开机/断链即回连，AT+BT_DISC 单次抑制 | 默认 1 | P3 | 🚧 已实现待实测 |
| `AT+BT_LIST` | 已绑定设备列表（SNV） | 无 | P3 | 🚧 已实现待实测 |
