...
- **标准库即可**：使用 `ESP32-BLE-Keyboard` 标准 Arduino 库，提供 boot
...
| # | 内容 | 产出 | 解锁的测试 |
|---|------|------|-----------|
| ① | 最简键盘 sketch（**标准 `ESP32-BLE-Keyboard` 库**，boot 报告）放 `tools/esp32c3_kbd/` | 替代 kbd 板跑通 loop/hardening | 现有全部双板回归的异构复测 |
