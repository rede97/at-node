...
- **ESP32-C3 variant**: `esp32/esp32_at_node/` — WiFi HTTP (`/at-node/*`) + MQTT + BLE HID keyboard, full AT command parity with CH582. **Flash ONLY via `esp32/esp32_at_node/build.ps1 -Port <COM>`** — its fqbn carries `CDCOnBoot=cdc`. Any other path (bare `arduino-cli`, Arduino IDE default) silently routes `Serial` to UART0 pads: native-USB COM port shows only the ROM boot log, AT dead, while WiFi/HTTP keep working. Symptom → reflash with build.ps1, do not debug the sketch.
