...
### F17. KEY_STR 映射表多两个字符,`(` 就是回车

- **现象**:bash 命令 `echo at-node-$(date +%s)` 打到一半自己换行执行了
- **根因**: shifted 数字行映射写成 `!@<#>$%^&*()`(12 个,混入 < >),
  全行错位:`$`→6、`(`→40=**Enter**、`)`→41=Esc
- **修法**:表改回 10 项 `!@#$%^&*()`
- **教训**:对称表(字符↔键位)必须逐项核对长度;特殊字符要纳入 KEY_STR 回归

### F18. 注入输入一律用 TAP/SEQ,裸 AT+KEY 只用于特例

- **现象**:测试/脚本里裸发 AT+KEY(按下),释放报告丢失 → 主机判定长按,
  终端被空格/字符洪流刷屏(2026-07-25 dongle→Linux 终端事故)
- **规则**:常规注入用 **AT+TAP**(原子按下+释放)或 **AT+KEY_STR/KEY_SEQ**
  (序列引擎自动配对 press/release);裸 AT+KEY 仅限"需要按住"的特例
  (修饰键、组合键按住阶段),且必须显式补 AT+KEY=0,0 释放
- **止血**:一旦卡住,补发 `AT+KEY=0,0`(全释放)×2-3
- **教训**:按下与释放必须原子化;跨命令的 press/release 配对是事故温床
