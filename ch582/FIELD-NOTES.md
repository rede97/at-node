...
### F19. 配对窗 60s 到点静默关闭 → "广播消失"假象

- **现象**:验证自定义 MAC 时反复扫不到设备,怀疑 MAC 没应用
- **根因**:配对窗(60s)到点自动关,空闲槽按策略停止广播;
  与 MAC 应用无关(实测自定义 MAC 和名字后缀全部正确上空中)
- **教训**:验证广播类功能时,先确认广播窗开着;把"开窗→立刻扫"写成原子操作
